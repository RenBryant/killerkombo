...
const resolvers = {
    Query: {
        ...
        getAllGames: async (parent, args, context) => {
            const { data: { results } } = await get(`https://api.rawg.io/api/games?page_size=3&key=` + API_KEY)
        ...
        getGenres: async (parent, args, context) => {
            const { data: { results } } = await get(`https://api.rawg.io/api/games?page_size=3&key=` + API_KEY)
            const genreData = results.map(({/* destructure results */ }) => ({/* make object of data to return */ }))
            return genreData
        },
        getGenreById: async (parent, { id }, context) => {
            const { data: { results } } = await get(`https://api.rawg.io/api/genres/${id}?key=` + API_KEY)
            const genreData = results.map(({/* destructure results */ }) => ({/* make object of data to return */ }))
            return genreData
        },
        getGamesBySearch: async (parent, { game }, context) => {
            const { data: { results } } = await get(`https://api.rawg.io/api/games?search=${game}&key=` + API_KEY)
